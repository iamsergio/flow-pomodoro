...
{
    Tag::Ptr leftTag = left.data(TagManager::TagPtrRole).value<Tag::Ptr>();
    Tag::Ptr rightTag = right.data(TagManager::TagPtrRole).value<Tag::Ptr>();
    Q_ASSERT(leftTag);
    Q_ASSERT(rightTag);
...
{
    QVariant variant = data(index(idx, 0), TagManager::TagPtrRole);
